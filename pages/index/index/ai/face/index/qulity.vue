<template>
  ...
  >
    ...
    >
      <template v-if="scoreMaxvision !== -1">
        <OcrResult
...
export default {
  ...
  data() {
    return {
      bgPic,
      headDescs: [
        "检测人脸质量。人脸色彩、清晰度、亮度、穿戴遮挡、正脸度、倾斜度",
      ],
      aiIntro: "检测人脸质量。人脸色彩、清晰度、亮度、穿戴遮挡、正脸度、倾斜度",
      imageDetectApiUrl,
      imageDetectTypeConfig,
      // 不显示hide = true的tab
      ocrDemotabList: [
  ...
  methods: {
    ...
    clearOcrResultInMixin() {
      ...
      this.scoreMaxvision = this.scoreBaidu = this.scoreTencent = -1;
    },
