      ...
      <div class="paddingTop special-wrap">
        ...
        >
          <template slot="result">
            ...
            <div class="g-flex" v-for="(item, index) in ocrResultData" :key="`ok${index}`">
              <span class="req-title">{{ index + 1 }}</span>
              <span class="req-val">{{item.words }}</span>
            </div>
          </template>
        </OcrDemo>
      </div>
...
export default {
  ...
  methods: {
    initData() {
      // 识别样图设置
      this.ocrDemotabList[0].samples = ocrSamplesReceipt;
    },
  },
  created() {
    this.initData();
